...
(function (OC, OCA) {
	...
	var COMMENT_TEMPLATE =
		...
		// '<div class="comment-time date has-tooltip" title="{{altDate}}">{{date}}</div>'+
		'<div class="comment-date"> ' +
		'	<p class="comment-date--date"> {{altDate}} </p>  ' +
		'	<p class="comment-date--time">  {{altTime}} </p>' +
		'</div>' +
		...
		/** @lends OCA.Comments.CommentsTabView.prototype */ {
			...
			_formatItem: function (commentModel) {
				...
				var data = _.extend({
					...
					altTime: moment(timestamp).format("HH:mm"),
					altDate: moment(timestamp).format("DD/MM/YY"),
